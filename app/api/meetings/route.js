import { jsonError, jsonSuccess } from "@/lib/api-response";
import { withErrorHandler } from "@/lib/error-handler";
import { requireAuth } from "@/lib/rbac";
import { checkRateLimit } from "@/lib/rateLimit";
import { connectDb } from "@/lib/mongodb";
import { getUserProfile } from "@/lib/firebase-admin";
import {
  ensureMeetingIndexes,
  getMeetingsForUser,
  getInstituteMeetingStats,
  computeMeetingStats,
  groupMeetingHistory,
} from "@/lib/services/meetingService";
import { processMeetingReminders } from "@/lib/services/meetingReminderService";

export const dynamic = "force-dynamic";
export const runtime = "nodejs";

export const GET = withErrorHandler(async (request) => {
  const decodedToken = await requireAuth(request);
  const role = decodedToken.role;

  if (!["parent", "teacher", "institute", "admin"].includes(role)) {
    return jsonError("Forbidden: Invalid role for meetings.", 403);
  }

  const ip = request.headers.get("x-forwarded-for") || "127.0.0.1";
  const rateLimitResult = await checkRateLimit(
    `meetings_get_${ip}_${decodedToken.uid}`
  );
  if (!rateLimitResult.allowed) {
    return jsonError("Too many requests. Please try again later.", 429);
  }

  const db = await connectDb();
  await ensureMeetingIndexes(db);
  await processMeetingReminders(db);

  const { searchParams } = new URL(request.url);
  const scope = searchParams.get("scope");
  const status = searchParams.get("status");
  const profile = await getUserProfile(decodedToken.uid);
  const instituteId = profile?.instituteId || profile?.uid;

  if (scope === "institute" && (role === "institute" || role === "admin")) {
    const stats = await getInstituteMeetingStats(db, instituteId);
    const meetings = await getMeetingsForUser(db, decodedToken.uid, role, {
      instituteId,
      limit: 50,
    });
    return jsonSuccess({ stats, meetings, history: groupMeetingHistory(meetings) });
  }

  const meetings = await getMeetingsForUser(db, decodedToken.uid, role, {
    status: status || undefined,
    instituteId: role === "institute" ? instituteId : undefined,
  });

  return jsonSuccess(meetings);
});
